...
class Agent(embodied.jax.Agent):
  ...
  def train(self, carry, data):
    ...
    metrics, (carry, entries, outs_loss, mets) = self.opt(
        self.loss, carry, obs, prevact, training=True, has_aux=True)
    metrics.update(mets)
    self.slowval.update()

    # Extract dynamics error for QU-Selector
    dyn_error = None
    if 'losses' in outs_loss and self.config.replay_context:
      losses_dict = outs_loss['losses']
      if 'dyn' in losses_dict:
        dyn_error = losses_dict['dyn']  # (B, T)
        metrics['qu_selector/dyn_error_mean'] = dyn_error.mean()
        metrics['qu_selector/dyn_error_std'] = dyn_error.std()
    ...
    if self.config.replay_context:
      ...
      # Add dynamics error for QU-Selector quality filtering
      if dyn_error is not None:
        updates['dyn_error'] = dyn_error
      outs['replay'] = updates
...
def imag_loss(
    ...
    actent=3e-4,
    slowreg=1.0,
    # Temporal smoothness constraint parameters
    smoothness_enable=False,
  ...
  if smoothness_enable:
    ...
    # Expand to same shape as rew (B*K, H+1)
    # smooth_score[i] corresponds to smoothness(a[i+1] - a[i]), should be added to rew[i+1]
    # So pad zeros at the beginning: [0, smooth_score[0], ..., smooth_score[H-1]]
    smooth_score_padded = jnp.concatenate([
        jnp.zeros_like(smooth_score[:, :1]),  # No smoothness reward for first step
        smooth_score
    ], 1)  # (B*K, H+1)
    
    # Modify reward: R_total = r_env + delta * smooth_score
    rew = rew + smoothness_weight * smooth_score_padded
    
    # Record smoothness metrics
    metrics['smoothness/score'] = smooth_score.mean()
    metrics['smoothness/reward_bonus'] = (smoothness_weight * smooth_score).mean()
