...
import supple.jax.context_block
import jax
...
class RSSM(nj.Module):
  ...
  def _dist(self, logits):
    out = supple.jax.outs.OneHot(logits, self.unimix)
    out = supple.jax.outs.Agg(out, 1, jnp.sum)
    return out


class Encoder(nj.Module):
  """Visual encoder integrating CASConv, SCFR, and AFNet.
  
  Architecture per layer:
  - CASConv: Channel-Attention Spatial Convolution for efficient feature extraction
  - SCFR + AFNet: Combined attention module with dual-path fusion (r=1/4)
  """
  ...
  compression_ratio: float = 0.25  # Compression ratio r=1/4
  pooling_type: str = 'att'
  fusion_mode: tuple = ('channel_add',)
  ...
  def __call__(self, carry, obs, reset, training, single=False):
    ...
    if self.imgkeys:
      ...
      for i, depth in enumerate(self.depths):
        # CASConv -> SCFR/AFNet  -> Norm -> Activation
        
        # Part 1: CASConv (Channel-Attention Spatial Convolution)
        if self.outer and i == 0:
          x = self.sub(f'casconv{i}', 'CASConv', depth, K, **self.kw)(x)
        elif self.strided:
          x = self.sub(f'casconv{i}', 'CASConv', depth, K, **self.kw)(x)
          B, H, W, C = x.shape
        ...
        if self.use_attention:
          x = self.sub(f'scfr{i}', 'AttentionModule', depth)(x)
...
class Decoder(nj.Module):
  ...
  def __call__(self, carry, feat, reset, training, single=False):
    ...
    if self.veckeys:
      ...
      kw = dict(**self.kw, outscale=self.outscale)
      outs = self.sub('vec', supple.jax.DictHead, spaces, outputs, **kw)(x)
      recons.update(outs)
    ...
    if self.imgkeys:
      ...
      for k, out in zip(self.imgkeys, jnp.split(x, split, -1)):
        out = supple.jax.outs.MSE(out)
        out = supple.jax.outs.Agg(out, 3, jnp.sum)
        recons[k] = out
